...
import argparse
import logging
from GatedRNN_prepro import prepro
from GatedRNN import GatedRNN
from GatedRNN_util import get_record_parser, evaluate_batch, get_batch_dataset, get_dataset, predict_batch
import warnings
...
def parse_args():
    ...
    model_settings.add_argument('--algo', choices=['GatedRNN'], default='GatedRNN',
    ...
    model_settings.add_argument('--capacity', type=int, default=60000,
    ...
    path_settings.add_argument('--prepared_dir', default='data/Gated/prepared',
                               help='the dir to store prepared ')
    path_settings.add_argument('--brc_dir', default='data/Gated/baidu',
                               help='the dir with preprocessed baidu reading comprehension data')
    path_settings.add_argument('--model_dir', default='data/Gated/models/',
                               help='the dir to store models')
    path_settings.add_argument('--result_dir', default='data/Gated/results/',
                               help='the dir to output the results')
    path_settings.add_argument('--summary_dir', default='data/Gated/summary/',
...
def modern_train(config, file_paths):
    ...
    model = GatedRNN(config, iterator, token_embeddings)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        sess.run(tf.assign(model.lr, tf.constant(config.learning_rate, dtype=tf.float32)))
        max_acc = 0
        ...
        for epoch in range(config.epochs):
            ...
            # train
            for i in range(train_total // config.batch_size):
                loss, train_op = sess.run([model.loss, model.train_op], feed_dict={handle: train_handle})
                # logger.info('Batch loss {}'.format(loss))
                bitx = i + 1
                n_batch_loss += loss
            ...
            eval_loss, eval_acc, summ = evaluate_batch(model, dev_total // config.batch_size, dev_eval_file,
                                                       sess, 'dev', handle, dev_handle)

            logger.info('Dev eval loss {}'.format(eval_loss))
            logger.info('Dev eval accuracy: {}'.format(eval_acc))
            for s in summ:
                writer.add_summary(s, epoch)
            writer.flush()
            filename = os.path.join(config.model_dir, "model_{}.ckpt".format(epoch))
            if eval_acc > max_acc:
                max_acc = eval_acc
...
def modern_predict(config, file_paths):
    ...
    logger.info('Building model...')
    model = GatedRNN(config, test_batch, token_embeddings, trainable=False)
    sess_config = tf.ConfigProto(allow_soft_placement=True)
    sess_config.gpu_options.allow_growth = True
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        sess.run(tf.assign(model.is_train, tf.constant(False, dtype=tf.bool)))
        predict_batch(model, total // config.batch_size + 1, eval_file, sess, 'predict', file_paths.final_file, logger)
...
def run():
    ...
    class FilePaths(object):
        def __init__(self):
            ...
            self.token2id_file = 'data/prepared/token2id.json'
            self.token_emb_file = 'data/prepared/token_emb.json'
            ...
            self.filtered_questions = os.path.join(args.prepared_dir, 'questions.json')
            self.predicted_answers = os.path.join(args.prepared_dir, 'test.predicted.json')
            self.final_file = os.path.join(args.result_dir, 'final.json')
