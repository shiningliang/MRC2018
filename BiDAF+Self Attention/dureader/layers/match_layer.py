...
class MatchLSTMBlockAttnCell(tc.rnn.LSTMBlockCell):
    """
    Implements the Match-LSTM attention cell
    """

    def __init__(self, num_units, context_to_attend):
        super(MatchLSTMBlockAttnCell, self).__init__(num_units, reuse=tf.AUTO_REUSE)
        self.context_to_attend = context_to_attend
        self.fc_context = tc.layers.fully_connected(self.context_to_attend,
                                                    num_outputs=self._num_units,
                                                    activation_fn=None)
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            new_inputs = tf.concat([inputs, attended_context,
                                    inputs - attended_context, inputs * attended_context],
                                   -1)
            return super(MatchLSTMBlockAttnCell, self).__call__(new_inputs, state, scope)
...
class MatchLSTMLayer(object):
    ...
    def match(self, passage_encodes, question_encodes, p_length, q_length):
        ...
        with tf.variable_scope('match_lstm', reuse=tf.AUTO_REUSE):
            cell_fw = MatchLSTMBlockAttnCell(self.hidden_size, question_encodes)
            cell_bw = MatchLSTMBlockAttnCell(self.hidden_size, question_encodes)
...
class AttentionFlowMatchLayer(object):
    ...
    def match(self, passage_encodes, question_encodes, p_length, q_length):
        ...
        with tf.variable_scope('bidaf', reuse=tf.AUTO_REUSE):
            sim_matrix_0 = tf.matmul(passage_encodes, question_encodes, transpose_b=True)
            context2question_attn = tf.matmul(tf.nn.softmax(sim_matrix_0, -1), question_encodes)
            b = tf.nn.softmax(tf.expand_dims(tf.reduce_max(sim_matrix_0, 2), 1), -1)
