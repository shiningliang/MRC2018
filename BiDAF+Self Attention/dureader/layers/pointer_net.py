...
def custom_dynamic_rnn(cell, inputs, inputs_len, initial_state=None):
    ...
    def loop_fn(t, prev_s, emit_ta, finished):
        ...
        if isinstance(cell, tc.rnn.LSTMBlockCell):
...
class PointerNetLSTMBlockCell(tc.rnn.LSTMBlockCell):
    """
    Implements the Pointer Network Cell
    """

    def __init__(self, num_units, context_to_point):
        super(PointerNetLSTMBlockCell, self).__init__(num_units, reuse=tf.AUTO_REUSE)
        self.context_to_point = context_to_point
        self.fc_context = tc.layers.fully_connected(self.context_to_point,
                                                    num_outputs=self._num_units,
                                                    activation_fn=None)
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__, reuse=tf.AUTO_REUSE):
            ...
            scores = tf.nn.softmax(logits, 1)
            attended_context = tf.reduce_sum(self.context_to_point * scores, axis=1)
            lstm_out, lstm_state = super(PointerNetLSTMBlockCell, self).__call__(attended_context, state)
        return tf.squeeze(scores, -1), lstm_state
    ...
    def decode(self, passage_vectors, question_vectors, init_with_question=True):
        ...
        with tf.variable_scope('pn_decoder', reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('fw', reuse=tf.AUTO_REUSE):
                fw_cell = PointerNetLSTMBlockCell(self.hidden_size, passage_vectors)
            ...
            with tf.variable_scope('bw', reuse=tf.AUTO_REUSE):
                bw_cell = PointerNetLSTMBlockCell(self.hidden_size, passage_vectors)
